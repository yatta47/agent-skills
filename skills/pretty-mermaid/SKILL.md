...
Japanese text is common in this environment, so the renderer defaults to `IPAGothic`.
If you want a different look, override it explicitly with `--font`.
...
- **Blog / pop visuals** → `candy-pop`
- **Soft technical visuals** → `pastel-tech`
- **Mermaid Live feel** → `mermaid-live-soft`
...
**Japanese-safe Font Override**:
```bash
node scripts/render.mjs \
  --input diagram.mmd \
  --font "IPAGothic" \
  --output diagram-ja.svg
```
